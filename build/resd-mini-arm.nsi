; 定义安装程序的名称和输出路径
OutFile "bin\resd-mini-arm-nsis.exe"
...
Section "Install"
  ...
  ; 将可执行文件复制到安装目录
  File "bin\resd-mini-arm.exe"
  File "windows\icon.ico"

  ; 创建开始菜单快捷方式
  CreateDirectory "$SMPROGRAMS\Resd Mini"
  CreateShortcut "$SMPROGRAMS\Resd Mini\Resd Mini.lnk" "$INSTDIR\resd-mini-arm.exe"
  CreateShortcut "$DESKTOP\Resd Mini.lnk" "$INSTDIR\resd-mini-arm.exe" "" "$INSTDIR\icon.ico"
...
Section "Uninstall"
  ...
  ; 删除安装目录中的所有文件
  Delete "$INSTDIR\resd-mini-arm.exe"
